...
std::shared_ptr<cell_type_parameters>  parameter_reader::read_cell_type_parameters(
    ...
    // TEST ADDED 1/16/25
    //Get the stiffness of the FENE spring
    auto FENE_stiffness_opt = get_string_value(cell_type_section, "FENE_stiffness"); 
    if(!FENE_stiffness_opt.has_value()) throw parameter_reader_exception("The XML markup \"FENE_stiffness\" of the cell "+cell_parameters->name_+" was not found in the parameter file.");
    cell_parameters->FENE_stiffness_ = std::stod(FENE_stiffness_opt.value());
