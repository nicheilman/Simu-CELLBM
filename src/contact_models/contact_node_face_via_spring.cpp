...
void contact_node_face_via_spring::run(const std::vector<cell_ptr> &cell_lst) noexcept
{
    ...
    const size_t nb_faces = std::accumulate(cell_lst.begin(), cell_lst.end(), 0, [](size_t acc, const cell_ptr &c)
                                            { return acc + c->get_nb_of_faces(); });
    face_lst_.clear();
...
#pragma omp parallel for
    for (size_t cell_id = 0; cell_id < cell_lst.size(); cell_id++)
    {
        ...
        const auto c1_cell_type = c1->get_cell_type();
            ...
            if (n.is_used())
            {
                ...
                for (face *f : grid_.voxel_lst_[voxel_id])
                {
                    ...
                    if (c1->get_id() != f->get_owner_cell()->get_id())
                    {
                        ...
                        if (aabb_intersection_check(face_aabb_pos, n.pos()))
                        {
                            // Get the cell owning the face
                            cell_ptr c2 = f->get_owner_cell();
			    const auto c1_cell_type = c1->get_cell_type_id();
                            const auto c2_cell_type = c2->get_cell_type_id();
                            
			    // Define some cell-type to cell-type lookup table
                            /* OurContactModel our_contact_model; */
			    const auto [adherence_strength, repulsion_strength] = OurContactModel::get_contact_strength(c1_cell_type, c2_cell_type);

                            // We know the cell type
                            apply_contact_forces(c1, n, f, adherence_strength, repulsion_strength);
                        }
                    }
                }
            }
...
void contact_node_face_via_spring::apply_contact_forces(cell_ptr c1, node &n, face *f, const double adherence_strength, const double repulsion_strength) const noexcept
{
...
// 11-13-24 Edit By N. Heilman
	//double cpa_f_to_node_proj = cpa_f_to_node_temp.dot(f->normal_);
	const vec3 cpa_f_to_node_proj = cpa_f_to_node * cpa_f_to_node.dot(f->normal_);
...
// Only compile this section if the faces store their contact energies
        ...
        // Reverse the direction of contact if the cell to which the face belongs is an static cell
        if (c1->get_cell_type_id() == 2 || c2->get_cell_type_id() == 2)
        {
	...
	std::cout<<c1->get_paired_cell_ptr()<<", "<<c2<<std::endl;
        ...
        if (adhesive_contact && min_squared_distance < interaction_cutoff_square_adhesion_)
        {
            ...
            {
                force_amplitude = 0; // adherence_strength * integration_region * (min_distance / hardening_distance_); /*  */

#if FACE_STORE_CONTACT_ENERGY
                adhesion_energy = 0.5 * adherence_strength * integration_region * min_distance * min_distance;
#endif
            }
            ...
            const vec3 adhesion_force_vector = cpa_f_to_node_proj * force_amplitude;
...
#if FACE_STORE_CONTACT_ENERGY
            repulsion_energy = 0.5 * repulsion_strength * integration_region * min_distance * min_distance;
            f->add_repulsion_energy(repulsion_energy);
#endif

            // The force that will be distributed on the nodes of the 2 faces
            const vec3 repulsion_force_vector = cpa_f_to_node * repulsion_strength * integration_region;
