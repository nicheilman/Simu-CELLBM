...
//Update cell pair for nuclei
if(cell_lst[i]->get_cell_type_id() == 3) cell_lst[i]->update_paired_cell_ptr(cell_lst);
...
std::optional<std::pair<cell_ptr, cell_ptr>>  cell_divider::divide_cell(
	...
	vec3 div_pla_norm;	
	if(c->get_cell_type_id() == 3){
        div_pla_norm = (c->get_paired_cell_ptr())->get_cell_division_axis();}
	else{div_pla_norm = c->get_cell_division_axis();}
	const vec3 division_plane_normal = div_pla_norm;
