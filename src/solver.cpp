...
solver::solver(
	...
	if(c->get_cell_type_id() == 3) c->set_paired_cell_ptr(cell_lst_[1]);
    }
...
//shared_ptr<LBM> lbm = make_shared<LBM>(0.1); //sim_parameters_.time_step_);
...
void solver::run_iteration() noexcept(false){
    ...
    if(!time_integrator_ptr_->is_step_tmp() && (1+iteration_) % 50 == 0) cell_divider::run(cell_lst_, sim_parameters_.min_edge_len_, *lmr_ptr_, max_cell_id_, verbose_);
...
lbm->run(time_integrator_ptr_->get_simulation_time(), cell_lst_);
lbm->write(time_integrator_ptr_->get_simulation_time());
...
//-----------------------------------------------------------------------
    ...
    #pragma omp parallel for
    for(size_t i = 0; i< cell_lst_.size(); i++){
	cell_lst_[i]->apply_internal_forces(sim_parameters_.time_step_, sim_parameters_.min_edge_len_);}

    //Update the positions of the nodes in parallel
    time_integrator_ptr_->update_nodes_positions(cell_lst_); //noexcept

    //Save the cell properties every 10 iterations in a csv file
    if(iteration_ % 10 == 0) statistic_writer_ptr_->write_data(iteration_, time_integrator_ptr_->get_simulation_time(), cell_lst_); //noexcept(false)
