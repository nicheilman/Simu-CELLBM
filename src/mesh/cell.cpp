...
void cell::initialize_random_properties() noexcept{
    ...
    //Set FENE_stiffness of the cell
//std::minstd_rand gen(std::chrono::system_clock::now().time_since_epoch().count());
//std::exponential_distribution<double> distribution(f.get_surface_tension());
/*
    if(cell_type_->FENE_stiffness_ != 0.){
        std::minstd_rand gen(std::chrono::system_clock::now().time_since_epoch().count());
        std::normal_distribution<double> distribution(0.0, cell_type_->FENE_stiffness_ );
        FENE_stiffness_ = 1.0e-4 + std::abs(distribution(gen));

	//Cap the value at the average
	if(FENE_stiffness_ > 1.0e-4 + 2 * cell_type_->FENE_stiffness_){
		FENE_stiffness_ = 1.0e-4 + 2 * cell_type_->FENE_stiffness_;
	}
	if(FENE_stiffness_ < 1.0e-4 - 2 * cell_type_->FENE_stiffness_){
                FENE_stiffness_ = 1.0e-4 - 2 * cell_type_->FENE_stiffness_;
        }
...
//---------------------------------------------------------------------------------------------------------
//Update the angle of each edge
void cell::update_all_edge_angles() noexcept{

    std::for_each(edge_set_.begin(), edge_set_.end(),
        [this](edge e) -> void {if(1) update_edge_angle(e);}
    );
...
//-------------------------------------------------------------------------------------------
// For updating the dihedral angle value stored in each edge member
void cell::update_edge_angle(edge e) noexcept{
	...
	//Get the nodes of the edge
        ...
        if (e0.dot(normal_1) > 0) theta = (2* M_PI) - theta;
        theta = M_PI - theta;
...
void cell::apply_surface_tension_and_membrane_elasticity(const double min_edge_len) noexcept{
    assert(cell_type_ != nullptr);
    ...
    for(face& f : face_lst_){
        ...
        if(f.is_used()){
            ...
            const double membrane_elasticity_factor =  -(cell_type_->area_elasticity_modulus_ / area_) * (1.0e1 * (area_ - area_memory_  / area_) + (1.0e-3 * (f.get_area() - f.get_area_memory()) / f.get_area() ));


    // Test for FENE Spring Force
        const vec3 e12 = n1.pos_ - n2.pos_;
        ...
        const double FENE_force_31 = (-1 * cell_type_->FENE_stiffness_)* e31.norm() / (1 - 0.050 * std::pow((e31.norm() / min_edge_len), 2) );
            ...
            const vec3 force_n1 = grad_pos_n1 * force_factor + e12 * (FENE_force_12 / e12.norm()) - e31 * (FENE_force_31 / e31.norm());
            const vec3 force_n2 = grad_pos_n2 * force_factor + e23 * (FENE_force_23 / e23.norm()) - e12 * (FENE_force_12 / e12.norm());
            const vec3 force_n3 = grad_pos_n3 * force_factor + e31 * (FENE_force_31 / e31.norm()) - e23 * (FENE_force_23 / e23.norm()) ;
...
//---------------------------------------------------------------------------------------------------------
void cell::push_force() noexcept{
    assert(cell_type_ != nullptr);

//const auto c = get_cell_type_id();
    const double mean = 0.0;
    const double stddev = 0.0e2;
    std::default_random_engine generator;
    generator.seed(get_id() + time(NULL));
    std::normal_distribution<double> dist(mean, stddev);
    vec3 noise_push(dist(generator), dist(generator), dist(generator));

if(get_cell_type_id() != 2){
    ...
    for(face& f : face_lst_){
	    ...
	    //double push_strength;
            vec3 face_push(0.0, 0.0, 0.0);


    // Define random generator with Gaussian distribution
    //const double mean = 0.0;
    //const double stddev = 1.0e1;
    //std::default_random_engine generator;
    //std::normal_distribution<double> dist(mean, stddev);
	    ...
	    face_push = push_force;
            ...
            //vec3 noise_push(dist(generator) * face_area / 3., dist(generator) * face_area / 3., dist(generator) * face_area / 3.); 
	    face_push = noise_push * face_area / 3. ;

            }

            // vec3 face_push(0.0, 0.0, push_strength * face_area / 3.); //, 0.0, 0.0);

            n1.add_force(face_push);
            n2.add_force(face_push);
            n3.add_force(face_push);	    
...
if(get_cell_type_id() == 0){
    if (centroid.dz() <= -0.75e-5){
	const vec3 transvect(0., 0., 1.5e-5);
        translate(transvect);	
    } else if (centroid.dz() >= 0.75e-5){
        const vec3 transvect(0., 0., -1.5e-5);
        translate(transvect);
		}
...
if(get_cell_type_id() != 5) {
        ...
        f_bend_x2 = grad_x2_ip * prefactor_1 +  grad_x2_theta * prefactor_2;
        f_bend_x3 = grad_x3_ip * prefactor_1 +  grad_x3_theta * prefactor_2;
}
else{
        //Assemble everything together to get the bending forces applied on the nodes of the hinge
        f_bend_x0 = (normal_1 + normal_2) * std::sin(theta - e.get_angle()) * 0.5 * avg_bending_stiffness;
        f_bend_x1 = (normal_1 + normal_2) * std::sin(theta - e.get_angle()) * 0.5 * avg_bending_stiffness;
        f_bend_x2 = normal_1 * std::sin(theta - e.get_angle()) * avg_bending_stiffness;
        f_bend_x3 = normal_2 * std::sin(theta - e.get_angle()) * avg_bending_stiffness;
}
...
//---------------------------------------------------------------------------------------------------------
void cell::apply_internal_forces(const double time_step, const double min_edge_len) noexcept{
    assert(cell_type_ != nullptr);
    assert(node_lst_.size() >= 4);
    assert(face_lst_.size() >= 4);
...
//push_force();
    ...
    apply_surface_tension_and_membrane_elasticity(min_edge_len);
...
//---------------------------------------------------------------------------------------------------------
//Return the eigen vector associated with the largest eigen value of the mass covariance matrix
double cell::get_Taylor_deformation() const noexcept{
    //First get the centroid of the cell
    const vec3 centroid = compute_centroid();
    //Compute the number of points that are used in the cell
    const double nb_points = static_cast<double>(get_nb_of_nodes());
    ...
    if(     std::abs(eigen_values.dx()) > std::abs(eigen_values.dy()) && std::abs(eigen_values.dx()) > std::abs(eigen_values.dz())){axis_max = eigen_values.dx();}
    else if(std::abs(eigen_values.dy()) > std::abs(eigen_values.dx()) && std::abs(eigen_values.dy()) > std::abs(eigen_values.dz())){axis_max = eigen_values.dy();}
    else{axis_max = eigen_values.dz();}

    if(     std::abs(eigen_values.dx()) < std::abs(eigen_values.dy()) && std::abs(eigen_values.dx()) < std::abs(eigen_values.dz())){axis_min = eigen_values.dx();}
    else if(std::abs(eigen_values.dy()) < std::abs(eigen_values.dx()) && std::abs(eigen_values.dy()) < std::abs(eigen_values.dz())){axis_min = eigen_values.dy();}
    else{axis_min = eigen_values.dz();}

    return (axis_max - axis_min)/(axis_max + axis_min);
}
//---------------------------------------------------------------------------------------------------------
...
void cell::regularize_face_angles(const face& f) noexcept{
    ...
    constexpr double min_angle = 20  * M_PI / 180.0;
    constexpr double max_angle = 160 * M_PI / 180.0;
