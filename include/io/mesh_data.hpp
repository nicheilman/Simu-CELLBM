...
//Print the centroids of the cells
    cell_data_mapper("centroid_dx", "float", [](cell_ptr c) -> std::string {
        vec3 centroid = c -> get_centroid();
        return format_number(centroid.dx(), "%.3e");
    }),
    cell_data_mapper("centroid_dy", "float", [](cell_ptr c) -> std::string {
        vec3 centroid = c -> get_centroid();
        return format_number(centroid.dy(), "%.3e");
    }),

    cell_data_mapper("centroid_dz", "float", [](cell_ptr c) -> std::string {
        vec3 centroid = c -> get_centroid();
        return format_number(centroid.dz(), "%.3e");
    }),

//Print the Taylor Deformation Parameter of the cells
    cell_data_mapper("Taylor Deformation", "float", [](cell_ptr c) -> std::string {
        return format_number(c -> get_Taylor_deformation(), "%.3e");
    }),
