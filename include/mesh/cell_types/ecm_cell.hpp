...
class ecm_cell: public cell {
    ...
    public:
        ...
        void apply_internal_forces(const double time_step, const double min_edge_len) noexcept override{
            //Only if the cell is not static we apply the internal forces such as the cell pressure, sureface tension etc
            if(!is_static_){cell::apply_internal_forces(time_step, min_edge_len);}
        }
