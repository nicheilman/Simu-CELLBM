...
class cell: public std::enable_shared_from_this<cell> {
	...
	//std::vector<face< face_area_lst_;

        //The centroid of the cell which is equal to the center of mass
        vec3 centroid_;

        double area_ = 0.;   //  Cell area
	double area_memory_ = 0.;    //  To remember previous iteration cell area
	...
	//The FENE_stiffness is drawn from an exponential distribution
	double FENE_stiffness_ = 0.;
...
void periodicBCs() noexcept;
    ...
    public:
        ...
        virtual void apply_internal_forces(const double time_step, const double min_edge_len) noexcept;
	...
	double get_Taylor_deformation() const noexcept;
	...
	float get_FENE_stiffness() const noexcept {return FENE_stiffness_;} 
	virtual cell_ptr get_paired_cell_ptr() const noexcept {return nullptr;}
	virtual void set_paired_cell_ptr(cell_ptr partner_cell_ptr) noexcept{};
	virtual void update_paired_cell_ptr(std::vector<cell_ptr> cell_lst) noexcept{};
void update_all_edge_angles() noexcept;
void update_edge_angle(edge) noexcept; 
